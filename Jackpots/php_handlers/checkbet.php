...
while ($row = $stmt->fetch()) {
    ...
    while ($row2 = $stmt2->fetch()) {
        if ($row2->result == NULL) {
            ...
            $sql4="UPDATE bets_table set total_odds = total_odds / ? ,possible_win= / ? where fixture_id=?";
    ...
    if ($status3 == 2 || $status == 0) {
        ...
        $sql5 = "UPDATE admintable set account_balance=account_balance-?,amount_paid_out=amount_paid_out+?,bets_won=bets_won+? where admin_id= ?";
        $stmt5 = $conn->prepare($sql5);
        $stmt5->execute(array($row4->possiblewin, $row4->possiblewin, 1, "0712345678"));
        ...
        $message = "betkipro bet id" . $id . " has won kes " . $row4->possiblewin;
    ...
    if ($status2 == 2) {
        ...
        $stmt3 = $conn->prepare($sql3);
        $stmt3->execute(array(1, "0712345678"));
    }
}
